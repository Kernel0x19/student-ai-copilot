...
export default function HowItWorksSection() {
  return (
    <section
      id="how-it-works"
      className="max-w-300 mx-auto px-10 py-24 border-t border-black/10 dark:border-white/8"
    >
      <div className="mb-16">
      ...
      <div className="relative">
        <div className="absolute left-9.75 top-10 bottom-10 w-px bg-black/10 dark:bg-white/8 hidden lg:block transition-all duration-500" />

        <div className="flex flex-col gap-0">
          {STEPS.map((step, index) => (
            <div
              key={step.number}
              className="relative grid grid-cols-1 lg:grid-cols-[80px_1fr] gap-6 group"
            >
              <div className="flex flex-col items-center gap-2 z-10">
                <div className="w-20 h-20 border border-black/10 dark:border-white/8 bg-white dark:bg-[#0F1117] group-hover:border-[#0C65D2]/40 group-hover:bg-[#0C65D2]/5 dark:group-hover:bg-[#0C65D2]/10 flex flex-col items-center justify-center gap-1 shrink-0 transition-all duration-500">
              ...
              <div
                className={`pb-12 ${index === STEPS.length - 1 ? "pb-0" : ""}`}
              >
                <div className="flex items-baseline gap-3 mb-2">
                  <h3 className="font-syne text-xl font-bold text-gray-900 dark:text-[#F0F4FF] transition-all duration-500">
                    {step.title}
                  </h3>
                  <span className="font-mono text-[11px] text-gray-400 dark:text-[#6B7280] border border-black/10 dark:border-white/8 px-2 py-0.5 transition-all duration-500">
                    {step.detail}
                  </span>
                </div>
                <p className="text-gray-500 dark:text-[#6B7280] text-sm leading-relaxed max-w-xl transition-all duration-500">
                  {step.description}
                </p>
              </div>
            </div>
