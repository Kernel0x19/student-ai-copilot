...
export default function TerminalCard() {
  const [visibleLines, setVisibleLines] = useState<
    { text: string; full: string; type: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const startLine = () => {
      ...
      intervalRef.current = setInterval(() => {
        ...
        setVisibleLines((prev) =>
          prev.map((l, i) =>
            i === lineIndex ? { ...l, text: line.text.slice(0, charIndex) } : l,
          ),
        );
  ...
  return (
    <div className="border border-black/10 dark:border-white/8 bg-gray-50 dark:bg-[#0F1117] overflow-hidden transition-all duration-500">
      <div className="bg-gray-100 dark:bg-[#161822] px-4 py-2.5 flex items-center gap-2 border-b border-black/10 dark:border-white/8 transition-all duration-500">
        <span className="w-2.5 h-2.5 rounded-full bg-[#ff5f57]" />
        <span className="w-2.5 h-2.5 rounded-full bg-[#febc2e]" />
        <span className="w-2.5 h-2.5 rounded-full bg-[#28c840]" />
        <span className="font-mono text-[11px] text-gray-400 dark:text-[#6B7280] ml-auto">
          edupilot — agent runtime
        </span>
      </div>
      <div className="p-5 font-mono text-[13px] leading-loose min-h-55">
        {visibleLines.map((line, i) => (
          <div key={i} className="flex items-baseline gap-2">
            <span className="text-[#0C65D2] select-none">›</span>
            <span className={colorClass(line.type)}>
              {line.text}
              {i === currentTyping && line.text !== line.full && (
                <span className="inline-block w-2 h-3.5 bg-[#0C65D2] ml-0.5 align-middle animate-[blink_1s_infinite]" />
              )}
            </span>
          </div>
        ))}
        {visibleLines.length === 0 && (
          <span className="inline-block w-2 h-3.5 bg-[#0C65D2] animate-[blink_1s_infinite]" />
        )}
      </div>
    </div>
  );
